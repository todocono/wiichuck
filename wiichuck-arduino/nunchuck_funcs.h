...
{
  /* original code for WII nunchuck
  ...
  Wire.beginTransmission(0x52);      // device address
    ...
    // read the extension type from the register block
    Wire.beginTransmission(0x52);
    Wire.write((uint8_t)0xFA);                    // extension type register
    Wire.endTransmission();
    Wire.beginTransmission(0x52);
    Wire.requestFrom(0x52, 6);               // request data from controller
    for (cnt = 0; cnt < 6; cnt++) {
    ...
    // send the crypto key (zeros), in 3 blocks of 6, 6 & 4.
    Wire.beginTransmission(0x52);
    Wire.write((uint8_t)0xF0);                    // crypto key command register
    Wire.write((uint8_t)0xAA);                    // sends crypto enable notice
    Wire.endTransmission();
    delay(1);
    Wire.beginTransmission(0x52);
    Wire.write((uint8_t)0x40);                    // crypto key data address
    for (cnt = 0; cnt < 6; cnt++) {
